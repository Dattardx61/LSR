...
TELEGRAM_BOT_TOKEN = '8115150808:AAEg24nddaDLUCjlMpHF49uhkIdUDFo1U18'
...
async def attack(update: Update, context: CallbackContext):
    ...
    if not is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="❌ **Access Denied!**\n⚠️ **You are not authorized to use this premium service.**", parse_mode='Markdown')
        return

    if is_on_cooldown(user_id):
        remaining = int(COOLDOWN_PERIOD - (time.time() - user_last_attack[user_id]))
        await context.bot.send_message(chat_id=chat_id, text=f"⏳ **Cooldown Active!**\n⚠️ Please wait {remaining} seconds before launching another attack.", parse_mode='Markdown')
        return

    args = context.args
    if len(args) != 3:
        await context.bot.send_message(chat_id=chat_id, text="❌ **Invalid Format!**\n⚠️ **Usage:** `/attack <ip> <port> <duration>`", parse_mode='Markdown')
        return
    ...
    await context.bot.send_message(chat_id=chat_id, text=( 
        ...
        f"💫 **Method:** `Udp-Api-Deploy`\n\n"
        f"🛡️ **Sit back and enjoy our elite service!** ⚔️"
    ), parse_mode='Markdown')
